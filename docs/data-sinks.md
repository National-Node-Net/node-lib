...
variable. The latter is advised to prevent repetitively specifying the broker for all sinks, including ianode-lib's 
internal sinks.
...
a `from ianode_lib.sinks import Serializers` and then referring to static methods on the `Serializers` class:
