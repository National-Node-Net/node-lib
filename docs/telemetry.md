...
node-lib provides metrics and traces that can be collected through OpenTelemetry.
...
node-lib will automatically count read, processed, and output records, but there are some situations where the decision 
made by an action cannot be captured automatically by node-lib.

* node-lib cannot automatically count when an adapter has read a record or processed a record that it then discards. 
* node-lib cannot automatically determine if a projector output a record or not.

A user must therefore tell node-lib when one of these events occur.
```python
...
Tracing of records through multiple actions is achieved by node-lib automatically injecting a `traceparent` 
